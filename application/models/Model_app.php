...
class Model_app extends CI_model{
    ...
    public function stok($id){
    	return $this->db->query("SELECT (IFNULL(sum(z.jml), 0)-IFNULL(sum(z.kurang), 0)) as stok 
    		FROM (SELECT sum(a.jml_terima) as jml, null as kurang 
    		FROM mu_pembelian_terima_detail a JOIN mu_pembelian_terima b ON a.id_pembelian_terima=b.id_pembelian_terima where a.id_barang='$id' 
    		UNION
    		SELECT null as jml, sum(a.jml_return) as kurang 
    		FROM mu_pembelian_return_detail a JOIN mu_pembelian_return b ON a.id_pembelian_return=b.id_pembelian_return where a.id_barang='$id'
    		UNION 
    		SELECT sum(tambah) as jml, sum(kurang) as kurang 
    		FROM mu_penyesuaian_detail where id_barang='$id'
    		UNION 
    		SELECT null as jml, sum(b.jumlah_jual*b.jumlah_satuan) as kurang 
    		FROM `mu_transaksi` a JOIN mu_transaksi_detail b ON a.id_transaksi=b.id_transaksi where b.id_barang='$id'
    		UNION
    		SELECT sum(a.jumlah_return) as jml, null as kurang 
    		FROM `mu_transaksi_return_detail` a JOIN mu_transaksi_detail b ON a.id_transaksi_detail=b.id_transaksi_detail 
    		JOIN mu_transaksi_return c ON a.id_transaksi_return=c.id_transaksi_return 
    		where c.status='selesai' AND b.id_barang='$id') as z");
    }
